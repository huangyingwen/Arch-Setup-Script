...
network_daemon_prompt() {
  if [ "${install_mode}" = 'server' ]; then
    ...
    1)
      ...
      ;;
    2)
      network_daemon='systemd-networkd'
      ;;
    *)
      output 'You did not enter a valid selection.'
      install_mode_selector
...
if [ "${install_mode}" = 'desktop' ]; then
  ...
  pacstrap /mnt sddm openssh pipewire-alsa pipewire-pulse pipewire-jack
...
if [ "${use_luks}" = '1' ]; then
  ...
  sed -i "s#quiet#root=${BTRFS} lsm=landlock,lockdown,yama,integrity,bpf mitigations=auto,nosmt spectre_v2=on spectre_bhi=on spec_store_bypass_disable=on tsx=off kvm.nx_huge_pages=force nosmt=force l1d_flush=on l1tf=full,force spec_rstack_overflow=safe-ret gather_data_sampling=force reg_file_data_sampling=on random.trust_bootloader=off random.trust_cpu=off efi=disable_early_pci_dma iommu=force iommu.passthrough=0 iommu.strict=1 slab_nomerge init_on_alloc=1 init_on_free=1 pti=on vsyscall=none ia32_emulation=0 page_alloc.shuffle=1 randomize_kstack_offset=on debugfs=off lockdown=confidentiality module.sig_enforce=1#g" /mnt/etc/default/grub
fi
